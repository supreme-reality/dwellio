...
@ActiveProfiles("test")
@Testcontainers
@Import(DocumentIT.TestJwtConfig.class)
class DocumentIT {
    ...
    @Test
    void completeWithoutObjectFails_withObjectUploads_andDelete() throws Exception {
        String ownerToken = mintToken("owner-" + UUID.randomUUID() + "@example.com", "Owner");
        String orgId = createOrg(ownerToken, "Doc Org " + UUID.randomUUID());
        String propertyId = createProperty(ownerToken, orgId, "House");

        MvcResult create = mockMvc.perform(post("/api/v1/properties/" + propertyId + "/documents")
                        .header("Authorization", "Bearer " + ownerToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"lease.pdf","contentType":"application/pdf"}
                                """))
        ...
        String documentId = com.jayway.jsonpath.JsonPath.read(
                create.getResponse().getContentAsString(), "$.document.id");
        String storageKey = com.jayway.jsonpath.JsonPath.read(
                create.getResponse().getContentAsString(), "$.document.storageKey");
        ...
        objectStorage.putObject(storageKey, "pdf-bytes".getBytes(), "application/pdf");
