...
@ActiveProfiles("test")
@Testcontainers
@Import(BedBlockIT.TestJwtConfig.class)
class BedBlockIT {
    ...
    @Test
    void blockRequiresReasonAndUnblockClearsColumns() throws Exception {
        String ownerToken = mintToken("owner-" + UUID.randomUUID() + "@example.com", "Owner");
        String orgId = createOrg(ownerToken, "Block Org");
        ...
        mockMvc.perform(get("/api/v1/beds/" + bedId).header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.availability").value("AVAILABLE"));

        mockMvc.perform(post("/api/v1/beds/" + bedId + "/block")
        ...
        mockMvc.perform(post("/api/v1/beds/" + bedId + "/block")
                        ...
                        .content("""
                                ...
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.availability").value("BLOCKED"));

        mockMvc.perform(post("/api/v1/beds/" + bedId + "/unblock")
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.availability").value("AVAILABLE"));
    }
