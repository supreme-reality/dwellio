...
@ActiveProfiles("test")
@Testcontainers
@Import(NoticeIT.TestJwtConfig.class)
class NoticeIT {
    ...
    @Test
    void noticeCrud_publish_stripsScript_andDelete() throws Exception {
        String ownerToken = mintToken("owner-" + UUID.randomUUID() + "@example.com", "Owner");
        String orgId = createOrg(ownerToken, "Notice Org " + UUID.randomUUID());
        String propertyId = createProperty(ownerToken, orgId, "House");

        MvcResult create = mockMvc.perform(post("/api/v1/properties/" + propertyId + "/notices")
                        .header("Authorization", "Bearer " + ownerToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  "title":"Water outage",
                                  "body":"Hello **tenants**. <script>alert(1)</script> Stay hydrated."
                                }
                                """))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.status").value("DRAFT"))
                .andExpect(jsonPath("$.body").value("Hello **tenants**.  Stay hydrated."))
                .andReturn();
        String noticeId = readId(create);

        mockMvc.perform(patch("/api/v1/notices/" + noticeId)
                        .header("Authorization", "Bearer " + ownerToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"title":"Water outage tonight"}
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.title").value("Water outage tonight"));

        mockMvc.perform(post("/api/v1/notices/" + noticeId + "/publish")
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("PUBLISHED"))
                .andExpect(jsonPath("$.publishedAt").isNotEmpty());

        mockMvc.perform(get("/api/v1/properties/" + propertyId + "/notices")
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].status").value("PUBLISHED"));

        mockMvc.perform(delete("/api/v1/notices/" + noticeId)
                        .header("Authorization", "Bearer " + ownerToken))
