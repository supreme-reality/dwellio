...
@ActiveProfiles("test")
@Testcontainers
@Import(DepositIT.TestJwtConfig.class)
class DepositIT {
    ...
    @Test
    void afterMoveInWithDeposit_ledgerHasReceiptAndBalanceMatches() throws Exception {
        String ownerToken = mintToken("owner-" + UUID.randomUUID() + "@example.com", "Owner");
        String orgId = createOrg(ownerToken, "Dep Org " + UUID.randomUUID());
        ...
        MvcResult moveInResult = mockMvc.perform(post("/api/v1/properties/" + propertyId + "/move-ins")
                ...
                .andReturn();
        String moveInId = com.jayway.jsonpath.JsonPath.read(moveInResult.getResponse().getContentAsString(), "$.id");
        String tenancyId = com.jayway.jsonpath.JsonPath.read(moveInResult.getResponse().getContentAsString(), "$.tenancyId");
        ...
        mockMvc.perform(get("/api/v1/tenancies/" + tenancyId + "/deposit")
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.tenancyId").value(tenancyId))
                .andExpect(jsonPath("$.balance").value(5000.00))
                .andExpect(jsonPath("$.totalReceipts").value(5000.00))
                .andExpect(jsonPath("$.totalDeductions").value(0.00))
                .andExpect(jsonPath("$.totalRefunds").value(0.00))
                .andExpect(jsonPath("$.currency").value("INR"));

        mockMvc.perform(get("/api/v1/tenancies/" + tenancyId + "/deposit/ledger")
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.balance").value(5000.00))
                .andExpect(jsonPath("$.entries", hasSize(1)))
                .andExpect(jsonPath("$.entries[0].type").value("RECEIPT"))
